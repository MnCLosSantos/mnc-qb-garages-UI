...
function populateVehicleList(garageLabel, vehicles) {
    ...
    vehicles.forEach((v) => {
        ...
        vehicleIcon.classList.add("fas", "fa-car", "vehicle-icon");
        vehicleItem.appendChild(vehicleIcon);
        ...
        financeInfo.innerHTML = `<i class="fas fa-dollar-sign"></i> ${
            v.balance && v.balance > 0 ? "Balance: $" + v.balance.toFixed(0) : "Paid Off"
        }`;
        ...
        driveButton.innerHTML = `<i class="fas fa-car-side"></i> ${status}`;
        ...
        if (status === "Out") {
            driveButton.style.background = "#4b5563";
        }
